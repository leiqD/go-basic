...
const (
	...
	timeBits    uint8 = 42                      //时间戳(毫秒)位数
	workerMax   int64 = -1 ^ (-1 << workerBits) // 节点ID的最大值，用于防止溢出
	numberMax   int64 = -1 ^ (-1 << numberBits) // 同上，用来表示生成id序号的最大值
	timeShift   uint8 = numberBits              // 时间戳向左的偏移量
	workerShift uint8 = timeBits + numberBits   // 节点ID向左的偏移量
	// 41位字节作为时间戳数值的话 大约68年就会用完
	epoch int64 = 1596211200000 // 起始时间：'2020-01-01'
)
...
func (w *Worker) GetId() int64 {
	...
	ID := int64((w.workerId << workerShift) | (now-epoch)<<timeShift | (w.number))
	return ID
}
